...
class Currency(object):
    def __init__(self, master):
        ...
        self.option = ["EUR", "USD", "CAD", "NZD", "CNY", "AUD", "DKK", "GBP","INR", "JPY", "NOK", "RUB", "SEK", "CHF", "ZAR"]
        ...
        self.from_menu = ttk.Combobox(self.selection_frame, value=self.option)
        self.from_menu.grid(column=0, row=1)
        self.from_menu.current(1)
        self.to_menu = ttk.Combobox(self.selection_frame, values=self.option)
    ...
    def converter(self):
        try:
            currency_result = str(round(self.currency.convert(float(self.amount_entry.get()), self.from_menu.get(), self.to_menu.get()), 2))
